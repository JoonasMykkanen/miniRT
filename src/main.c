...
/*   Updated: 2023/09/18 12:25:57 by djames           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray create_shadow_ray(t_vector surface_point, t_vector light_source_position) {
    t_ray shadow_ray;

    // Calculate the dir vector from the surface point to the light source
    shadow_ray.dir.x = light_source_position.x - surface_point.x;
    shadow_ray.dir.y = light_source_position.y - surface_point.y;
    shadow_ray.dir.z = light_source_position.z - surface_point.z;

    // Normalize the shadow ray dir
	
    float length = dotProduct(shadow_ray.dir, shadow_ray.dir);
	length = sqrtf(length);
	// length = sqrt(shadow_ray.dir.x * shadow_ray.dir.x + 
    //                     shadow_ray.dir.y * shadow_ray.dir.y + 
    //                     shadow_ray.dir.z * shadow_ray.dir.z);
    ...
    // Set the origin of the shadow ray to the surface point
    shadow_ray.orig = surface_point;

    return shadow_ray;
}

int is_in_shadow(t_vector surface_point, t_vector light_source_position, t_sphere* spheres, int num_spheres) {
    ...
    for (int i = 0; i < 3; i++) {
		if(i == num_spheres)
        {
			...
			t = hit_sphere(spheres[i].center, spheres[i].radius, shadow_ray);
        if (t > 0) {
...
void	render(void *param)
{
	...
	uint32_t color;
	t_vector intersect;// this is for the shadow 
	int flag; // if it hits 
	...
	for (int j = 0; j < HEIGHT; ++j) {
        for (int i = 0; i < WIDTH; ++i) {
			...
			// if (pt < t66) {
				// intersect = ray_at(data->scene.ray, pt);
				
				// flag = is_in_shadow(intersect, data->scene.light.position, data->scene.spheres, a2);
			// 	if(flag == 1)
			// 	{
			// 		color = 0x000000ff;
			// 	}
